...
# ---------------------------------------------------------
ticker = 'TSLA'
print(f"Fetching {ticker} data to train Pure LSTM Model...")
...
# UPDATED: Removed EGARCH_Vol, keeping FracDiff and standard volatility features
feature_cols = ['FracDiff_Feature', 'Log_Return', 'Rolling_Vol', 'DayOfWeek', 'Month', 'DayOfYear']
...
vol_forecast_df = pd.DataFrame({
    ...
    'LSTM_VOL': y_pred.flatten()
})

# ---------------------------------------------------------
# 7. Multi-Contract Validation & Pricing
# ---------------------------------------------------------
print("\nValidating Pure LSTM Model against Real TSLA Option Prices...")
...
print("\n--- Aggregate Pure LSTM + MJD Pricing Error ---")
...
# Plot actual model predictions (changed to green for the Pure LSTM)
plt.scatter(actuals, predictions, alpha=0.4, color='green', edgecolor='k', label='Pure LSTM+MJD Forecasts')

plt.title('Pure LSTM Model Validation: Actual vs. Predicted Prices (Top 20 Contracts)', fontsize=14)
