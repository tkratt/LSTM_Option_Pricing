...
for ticker, name in assets.items():
    # Fetching individually to avoid MultiIndex column complexities
    ticker_data = yf.download(ticker, start=start_date, end=end_date)
    df[name] = ticker_data['Close']

print("Fetching Fed Funds Rate from FRED...")
df_fred = web.DataReader('DFF', 'fred', start_date, end_date)
df['fed_funds_rate'] = df_fred['DFF']

# Forward-fill missing values (handles differing market holidays and weekend rates)
df = df.ffill()

# Calculate daily log returns for the required assets
df['Log_Return'] = np.log(df['SPX'] / df['SPX'].shift(1))
df['QQQ_logret'] = np.log(df['QQQ'] / df['QQQ'].shift(1))
...
# Drop the initial NaNs created by the shift operation
df = df.dropna()
...
# Extract the numpy arrays for scaling (features is now 2D)
...
# fit_transform now scales all 7 features appropriately
...
print(f"X_train shape: {X_train.shape}") # Will now output (Samples, 21, 7)

# ---------------------------------------------------------
# 3. Model Architecture & Training
# ---------------------------------------------------------
model = Sequential()
# Architecture untouched: shape[2] dynamically pulls the 7 features
